...
public class KakaoPayService {
    public ReadyResponse payReady(int totalPrice) {

        Map<String, String> parameters = new HashMap<>();
        parameters.put("cid", "TC0ONETIME");                                    // 가맹점 코드(테스트용)
        parameters.put("partner_order_id", "1234567890");                       // 주문번호
        parameters.put("partner_user_id", "roommake");                          // 회원 아이디
        parameters.put("item_name", "청소아이하니");                                      // 상품명
        parameters.put("quantity", "1");                                        // 상품 수량
        parameters.put("total_amount", String.valueOf(totalPrice));             // 상품 총액
        parameters.put("tax_free_amount", "0");                                 // 상품 비과세 금액
//        parameters.put("approval_url", "http://10.10.10.152:8080/api/v1/mypage/order/pay/completed"); // 결제 성공 시 URL
        parameters.put("approval_url", "http://www.cleanaido.shop/api/v1/mypage/order/pay/completed"); // 결제 성공 시 URL
    ...
    private HttpHeaders getHeaders() {
        ...
        headers.set("Authorization", "SECRET_KEY DEV328967FE3301A9F8E8AE7945FB7A8FFCFA290");
